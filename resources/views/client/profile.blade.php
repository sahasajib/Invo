        ...
        <div class="max-w-5xl mx-auto">
            <div class="">
                ...
                <div class="grid grid-cols-4 gap-5 mt-6">
                        <x-card text="Total Tasks" route="{{route('task.index')}}?client_id={{ $client->id }}" :count="count($client->tasks)" class="bg-gradient-to-tr
                    from-teal-200 to-yellow-500 rounded-md"></x-card>

                        <x-card text="Pending Tasks" route="{{route('task.index')}}?client_id={{ $client->id }}&status=pending" :count="count($pending_tasks)" class="bg-gradient-to-tr
                    from-teal-200 to-yellow-500 rounded-md"></x-card>

                        <x-card text="Total Invoice" route="{{route('invoice.index')}}?client_id={{ $client->id }}" :count="count($client->invoices)" class="bg-gradient-to-tr
                    from-teal-200 to-yellow-500 rounded-md"></x-card>

                        <x-card text="Paid Invoice" route="{{route('invoice.index')}}?client_id={{ $client->id }}&status=paid" :count="count($paid_invoices) " class="bg-gradient-to-tr
                    from-teal-200 to-yellow-500 rounded-md"></x-card>
                </div>
            </div>
        </div>
